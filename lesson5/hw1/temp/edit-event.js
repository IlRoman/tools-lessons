    ...
    activeEvents[i].addEventListener('click', function () {
      for (let j = 0; j < arr.length; j++) {
        if (event.target.dataset.id == arr[j]._id) {
          displayPopup(arr[j]);
          deleteButtonOnclick(arr[j]); // edit event

          let editButton = document.querySelector('.submit-button '); // удалить все ивент-листенеры

          let new_element = editButton.cloneNode(true);
          editButton.parentNode.replaceChild(new_element, editButton);
          new_element.addEventListener('click', editEvent.bind(editButton, arr[j]));
        }
      }
    });
  }
...
function displayPopup(editedEvent) {
  ...
  let description = document.querySelector('.description-input '); // вносим title в popup

  name.value = editedEvent.name + ''; // вносим дату в popup (input type="date")

  let startMonth = editedEvent.startDate.getMonth() + 1 < 9 ? `0${editedEvent.startDate.getMonth() + 1 + ''}` : `${editedEvent.startDate.getMonth() + 1}`;
  let startYear = (editedEvent.startDate + '').split(' ')[3];
  let startDay = (editedEvent.startDate + '').split(' ')[2];
  startDate.value = `${startYear}-${startMonth}-${startDay}`; // вносим время в popup
  ...
  let endMonth = editedEvent.endDate.getMonth() + 1 < 9 ? `0${editedEvent.endDate.getMonth() + 1 + ''}` : `${editedEvent.endDate.getMonth() + 1}`;
  let endYear = (editedEvent.endDate + '').split(' ')[3];
  let endDay = (editedEvent.endDate + '').split(' ')[2];
  endDate.value = `${endYear}-${endMonth}-${endDay}`; // вносим description в popup

  description.value = editedEvent.description;
}

function editEvent(obj) {
  deleteFromServer(obj._id).then(() => renderDates());
