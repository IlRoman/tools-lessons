    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (eventDay[i].innerHTML == endTime.getDate()) {
        ...
        if (starttime.getDate() !== endTime.getDate()) {
          ...
          activeEventLong.style.height = `${1440 - (1440 - (endTime + '').split(' ')[4].split(':')[0] * 60 + +(endTime + '').split(' ')[4].split(':')[1])}px`;
...
export function renderEvents(arr) {
  if (arr.length !== 0) {
    for (let i = 0; i < arr.length; i++) {
      ...
      displayEvent(arr[i].startDate, arr[i].endDate, arr[i].name, arr[i].description, arr[i]._id, arr[i].color);
    }
